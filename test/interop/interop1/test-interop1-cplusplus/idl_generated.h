#ifndef _idl_generated_h_214ad32b_guard
#define _idl_generated_h_214ad32b_guard
...
struct publishable_STRUCT_BasicTypes : public ::globalmq::marshalling::impl::StructType
{
	...
	RetT parse( ParserT& parser, T& t )
	{
		...
		if constexpr( has_any_notifier_for_anInt || reportChanges || has_update_notifier )
		{
		...
		if constexpr( has_any_notifier_for_anUInt || reportChanges || has_update_notifier )
		{
		...
		if constexpr( has_any_notifier_for_aReal || reportChanges || has_update_notifier )
		{
		...
		if constexpr( has_any_notifier_for_aString || reportChanges || has_update_notifier )
		{
...
struct publishable_STRUCT_point3D : public ::globalmq::marshalling::impl::StructType
{
	...
	RetT parse( ParserT& parser, T& t )
	{
		...
		if constexpr( has_any_notifier_for_x || reportChanges || has_update_notifier )
		{
		...
		if constexpr( has_any_notifier_for_y || reportChanges || has_update_notifier )
		{
		...
		if constexpr( has_any_notifier_for_z || reportChanges || has_update_notifier )
		{
...
struct publishable_STRUCT_AggregateType : public ::globalmq::marshalling::impl::StructType
{
	...
	RetT parse( ParserT& parser, T& t )
	{
		...
		::globalmq::marshalling::impl::parsePublishableStructEnd( parser );
		if constexpr( has_any_notifier_for_lastValue || reportChanges || has_update_notifier )
		{
...
//**********************************************************************
// MESSAGE "message_two" NONEXTENDABLE Targets: GMQ (Alias of struct_one)
...
//**********************************************************************
// MESSAGE "message_three" NONEXTENDABLE Targets: JSON (Alias of struct_one)
...
public:
	...
	Mock_WrapperForPublisher( ArgsT&& ... args ) : t( std::forward<ArgsT>( args )... ), composer( buffer ) {}
	...
	void compose( ComposerType& composer )
	{
		...
		::globalmq::marshalling::impl::publishableStructComposeInteger( composer, t.id, "id", false );
...
public:
	...
	template<class ... ArgsT>
	Mock_WrapperForSubscriber( ArgsT&& ... args ) : t( std::forward<ArgsT>( args )... ) {}
	const T& getState() { return t; }
			...
			switch ( addr[0] )
			{
				...
				case 1:
				{
					if ( addr.size() == 1 ) // we have to parse and apply changes of this child
					{
						...
						{
							if constexpr ( reportChanges || has_update_notifier )
								changed = publishable_STRUCT_BasicTypes::parse<ParserT, decltype(T::basic), bool>( parser, t.basic );
							else
					...
					else // let child continue parsing
					{
						...
						else if constexpr( has_void_update_notifier_for_basic )
						{
							...
							changed = publishable_STRUCT_BasicTypes::parse<ParserT, decltype(T::basic), bool>( parser, t.basic, addr, 1 );
						else
				...
				case 2:
				{
					if ( addr.size() == 1 ) // we have to parse and apply changes of this child
					{
						...
						{
							if constexpr ( reportChanges || has_update_notifier )
								changed = publishable_STRUCT_AggregateType::parse<ParserT, decltype(T::aggregate), bool>( parser, t.aggregate );
							else
					...
					else // let child continue parsing
					{
						...
						else if constexpr( has_void_update_notifier_for_aggregate )
						{
							...
							changed = publishable_STRUCT_AggregateType::parse<ParserT, decltype(T::aggregate), bool>( parser, t.aggregate, addr, 1 );
						else
...
public:
	virtual StateConcentratorBase<InputBufferT, ComposerT>* createConcentrator( uint64_t typeID )
	{
		...
		{
			case 5:
				return new Mock_WrapperForConcentrator<structures::Mock, InputBufferT, ComposerT>;
			case 6:
...
//**********************************************************************
// STRUCT "point3D" NONEXTENDABLE Targets: JSON GMQ (3 parameters)
//  1. INTEGER x (REQUIRED)
//  2. INTEGER y (REQUIRED)
...
//**********************************************************************
// STRUCT "struct_one" NONEXTENDABLE Targets: JSON GMQ (10 parameters)
...
//  6. VECTOR< STRUCT point> sixthParam (REQUIRED)
//  7. REAL seventhParam (REQUIRED)
//  8. STRUCT point eighthParam (REQUIRED)
...
void STRUCT_struct_one_compose(ComposerT& composer, Args&& ... args)
{
	...
	using arg_6_type = NamedParameterWithType<::globalmq::marshalling::impl::VectorOfMessageType, sixthParam_Type::Name>;
	using arg_7_type = NamedParameterWithType<::globalmq::marshalling::impl::RealType, seventhParam_Type::Name>;
	using arg_8_type = NamedParameterWithType<::globalmq::marshalling::impl::MessageType, eighthParam_Type::Name>;
...
#endif // _idl_generated_h_214ad32b_guard
